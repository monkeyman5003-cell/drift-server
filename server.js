...
// a shared world seed per room so both players generate the identical city
const roomSeeds = new Map(); // Map<roomName, number>

function getRoom(name) {
  if (!rooms.has(name)) rooms.set(name, new Map());
  if (!lastState.has(name)) lastState.set(name, new Map());
  if (!roomSeeds.has(name)) roomSeeds.set(name, Math.floor(Math.random() * 1e9));
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'join') {
      ...
      // tell the joiner their slot, the room count, and the shared world seed
      ws.send(JSON.stringify({ type: 'joined', slot: ws.slot, count: room.size, seed: roomSeeds.get(roomName) }));
    ...
    // --- a player hit the other player (relay the damage) ---
    if (msg.type === 'hit' && ws.roomName && ws.playerId) {
      broadcast(ws.roomName, ws.playerId, { type: 'hit', amount: msg.amount });
      return;
    }

    // --- an NPC was killed/destroyed (relay so it syncs) ---
    if (msg.type === 'npc' && ws.roomName && ws.playerId) {
      broadcast(ws.roomName, ws.playerId, { type: 'npc', kind: msg.kind, netId: msg.netId });
      return;
    }
  });
  ...
  ws.on('close', () => {
    if (ws.roomName && rooms.has(ws.roomName)) {
      ...
      if (room.size === 0) { rooms.delete(ws.roomName); lastState.delete(ws.roomName); roomSeeds.delete(ws.roomName); }
    }
  });
});
